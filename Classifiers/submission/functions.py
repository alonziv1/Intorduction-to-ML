...
def pre_tuning_svm_clf(X_train, y_train):
    ...
    for iteration in range(5):
        ...
        scores.append(score)
        visualize_clf.visualize_clf(svm_clf, X_train, y_train, "decision regions pre-tuning - iteration  " + str(iteration +1)+ ', score is:' +str(score) , "PCR_05" , "sugar_levels" )
...
def tuning_C_hyperparameter(X_train,y_train, start ,end):
    ...
    train_C_opt = 0
    validation_C_opt = 0
    power_values = np.arange(start,end, step=0.1)
    C_range = [10**e for e in power_values]
    C_train_accuracies= []
...
def train_and_test_tuned_C_svm_clf (X_train, y_train, X_test, y_test ,optimal_C  ):
    ...
    visualize_clf.visualize_clf(svm_clf, X_test.to_numpy(), y_test.to_numpy(), "decision regions with optimal C ", "PCR_05" , "sugar_levels" )
